...
public class CSJ {
    ...
    CSJ(int ff, int maxItr, int dimensiion, int numPopulation, double L, double U) {
        f = ff;
        n = numPopulation;
        d = dimensiion;
        LB = L;
   ...
   public void GenerateCuckoo(int iteration){
        ...
        for (int i = 0; i < population[selected].length ; i++) {
            while(numberofAttemps>=0 && !(temp>=LB && temp<=UB) ){
             temp= population[selected][i]+random3.nextDouble()* population[0][i] + random4.nextDouble()* population[n-1][i] ; 
             cuckoo[i]= temp; 
             numberofAttemps--;
            }
        }
