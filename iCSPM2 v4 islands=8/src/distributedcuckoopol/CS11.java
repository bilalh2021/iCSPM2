...
public class CS11 {
    ...
    CS11(int ff, int maxItr, int dimensiion, int numPopulation, double L, double U) {
        f = ff;
        n = numPopulation;
        d = dimensiion;
        LB = L;
...
double result = LO + (random * (UO - LO));
    ...
    public void GenerateCuckoo(int iteration) {
        ...
        for (int i = 0; i < population[selected].length; i++) {
            while (numberofAttemps >= 0 && !(temp >= LB && temp <= UB)) {
                ...
                if (random.nextDouble() < 0.5) // if <.5 subtract else if >.5 add 
                {
                    // generate random number between [-1,1]
                    if (population[selected][i] - alpha >= LB){ // check the range [-5,5]
                    cuckoo[i] = population[selected][i] - alpha;
                    }
                 else if (population[selected][i] + alpha <= UB) //double alpha=(double(rand())/RAND_MAX)*2 -1; // generate random number between [-1,1]
                 // check the range [-5,5]
                 {
                    cuckoo[i] = population[selected][i] + alpha;
                    }
