...
public class CS1 {
  ...
  CS1(int ff, int maxItr, int dimensiion, int numPopulation, double L, double U) {
        f = ff;
        n = numPopulation;
        d = dimensiion;
        LB = L;
...
private double levyFlight(double xmin ) {
     ...
     Random random2 = new Random();
        ...
        for (int i = 0; i < population[selected].length ; i++) {
            while(numberofAttemps>=0 && !(temp>=LB && temp<=UB) ){
             temp= population[selected][i]+levyFlight(iteration/MaxGeneration);  
             cuckoo[i]= temp; 
             numberofAttemps--;
            }
        }
